...
def create_index():
    ...
    with get_connection() as conn:
        conn.execute(text("CREATE INDEX IF NOT EXISTS buildings_geom_idx ON buildings USING GIST(geom)"))
        conn.commit()
    return {"status": "created", "index": "buildings_geom_idx"}

def drop_index():
    """Drop spatial index"""
    with get_connection() as conn:
        conn.execute(text("DROP INDEX IF EXISTS buildings_geom_idx"))
        conn.commit()
    return {"status": "dropped", "index": "buildings_geom_idx"}
